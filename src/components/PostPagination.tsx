...
export default function PostPagination({ listData, pagingFn } : {
  ...
  const viewPagination = (count : number) => {
    return Array.from({ length: count }).map((_,index) => (
      <span key={"pagingNum0" + (index + 1)} className="cursor-pointer pagingView" 
        onClick={(e) => pagingClick(e,`${index + 1}`)} >{index + 1}</span>
    ))
  }
