...
export default function SideBar() {
  ...
  const optionList: OptionList = {
    ...
    drivetrain: '구동 타입',
    passenger: `${
      titdata[Number(param.model) - 1]?.name === 'g80' ? '스포츠 패키지' : '시트 구성'
    }`,
    exterior: '외장 컬러',
    interior: '내장디자인 & 컬러',
    garnish: '내장가니쉬',
              ...
              {upperName !== 'NEOLUN CONCEPT' ? (
                <>
                  ...
                  {item.id === Number(param?.model) ? (
                    <div className="toggleModelOptions">{optionViewNames(upperName)}</div>
                  ) : (
                    <></>
                  )}
                </>
          ...
          <div className="mobileMenuWrap">
            ...
            <article>
              ...
              <div className="toggleWrap on" ref={toggleBbsRef}>
                <Link href="/drive" onClick={() => setModalOn(false)}>
                  전시시승
                </Link>
                <Link href="/qna" onClick={() => setModalOn(false)}>
                  고객지원
                </Link>
                <Link href="/info" onClick={() => setModalOn(false)}>
                  공지사항
                </Link>
              </div>
            </article>
