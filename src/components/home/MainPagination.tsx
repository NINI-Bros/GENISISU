...
export default function MainPagination() {
  ...
  useEffect(() => {
    ...
    if (currScroll <= viewHeight * 0.5) {
      ...
      currScroll > viewHeight * 0.5 - minusScroll &&
      currScroll < viewHeight * 1.5 - minusScroll
    ) {
      setActivePage(2);
    } else if (
      currScroll > viewHeight * 1.5 - minusScroll &&
      currScroll < viewHeight * 2.5 - minusScroll
    ) {
      setActivePage(3);
    } else if (
      currScroll > viewHeight * 2.5 - minusScroll &&
      currScroll < viewHeight * 3.5 - minusScroll
    ) {
      setActivePage(4);
    } else if (
      currScroll > viewHeight * 3.5 - minusScroll &&
      currScroll < viewHeight * 4.5 - minusScroll
    ) {
      setActivePage(5);
    }
  ...
  return (
    <ul className="main_pagination" ref={pagingRef}>
      ...
      <li className={activePage === 1 ? 'on' : ''}>
        <a href="#none" onClick={(e) => liPaging(e, 1)}></a>
      </li>
      <li className={activePage === 2 ? 'on' : ''}>
        <a href="#none" onClick={(e) => liPaging(e, 2)}></a>
      </li>
      <li className={activePage === 3 ? 'on' : ''}>
        <a href="#none" onClick={(e) => liPaging(e, 3)}></a>
      </li>
      <li className={activePage === 4 ? 'on' : ''}>
        <a href="#none" onClick={(e) => liPaging(e, 4)}></a>
      </li>
      <li className={activePage === 5 ? 'on' : ''}>
        <a href="#none" onClick={(e) => liPaging(e, 5)}></a>
      </li>
    </ul>
  );
}
