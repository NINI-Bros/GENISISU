...
export default function ModelGnb({ params }: { params: { model: string } }) {
  ...
  return (
    <nav className="flex items-end absolute top-[40px] left-[80px] z-[6] w-[calc(100vw-_100px)] h-[70px] overflow-hidden max-[1366px]:hidden">
      ...
      >
        ...
        <div className="col-start-2 row-start-1 row-span-2 self-center justify-self-end w-[10px] h-[20px] relative">
          <Image
            ...
            alt="화면 상단에서 추가로 모델을 선택할 수 있는 보조 GNB의 클릭 버튼 입니다."
          />
        </div>
      </button>
      <div
      ...
      >
        <Swiper
          className="subGnb flex absolute w-[1200px] overflow-x-scroll overflow-y-hidden  opacity-1"
          spaceBetween={0}
          slidesPerView={6}
        >
            ...
            return (
              <SwiperSlide key={name} className="box-border">
                <Link
                  ...
                  key={name}
                  className="flex flex-col font-Hyundai-sans w-full h-[70px] pt-[5px]  border-[#777] border-[1px] bg-black px-[20px] hover:bg-[#333]"
                >
                  <span className="col-start-1 text-[25px] w-full self-start text-left font-medium ">
                    {titName.toUpperCase()}
                  </span>
                  <span className="col-start-1 text-[12px] self-start text-left text-nowrap ">
                    {subName ? subName.toUpperCase() : 'STANDARD'}
                  </span>
                </Link>
              </SwiperSlide>
            );
