    ...
    <section className="w-screen h-[calc(100vh-100px)] grid grid-cols-[450px_auto] gap-x-[4rem] bg-black z-10">
      <article className="w-[90%] max-h-full col-start-2 grid-cols-1 flex flex-col justify-items-center mt-[40px] pb-[100px] gap-y-[20px] overflow-hidden justify-between">
        ...
        <div className='grid grid-cols-[auto_1fr_1fr] gap-y-[10px] gap-x-[3rem] self-center'>
          <h2 className="text-[50px] font-Hyundai-sans self-center justify-self-center px-[10px] font-black">{modelName && uppercaseName}</h2>
        ...
        <div className="flex flex-col w-[120px] gap-y-[10px] mt-[0px] col-span-2 self-start absolute top-[620px] left-[80px] ">
          ...
          <button className='mainBtn w-full h-[37px] font-normal border-2 border-white bg-white text-black text-[16px]' style={{fontFamily:"Pretendard"}} onClick={clickNext}>
            다음
          </button>
        </div>
